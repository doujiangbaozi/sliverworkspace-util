...
class GermlineSnvAnnotationFilter(object):
    ...
    def __init__(self):
        ...
        try:
            self.opts = getopt.getopt(
                sys.argv[1:],
                "a:o:v:i:e:dh",
                ["annovar=", "database=", "out=", "vcf=","min-depth=", "document", "help"]
            )
        ...
        for arg, value in self.opts[0]:
            if arg == "-v" or arg == "--vcf":
                ...
                if os.path.isfile(value):
                    ...
                    print('[--database]参数值 %s 不是一个有效的数据文件' % (value))
                    sys.exit()
                ...
                import GermlineSnvAnnotationFilter
                help(GermlineSnvAnnotationFilter)
                sys.exit()
        ps_stats = False
        if self.database is None:
            print('[--database]参数值不能为空')
            ps_stats = True
        if self.vcf is None:
    ...
    def doProcess(self, vcf, anno, out):
        ...
        if (os.path.exists(vcf)) and (os.path.getsize(vcf) > 0) and (os.path.exists(anno)) and (os.path.getsize(anno) > 0):
            ...
            if not result_data.empty:
                ...
                # 匹配突变数据库中的基因
                result_data['BLACKLIST'] = result_data.apply(
                    lambda row: self.matchBlackList(row['Gene.refGene']), axis=1)
                result_data = result_data[result_data['BLACKLIST'] == True]
                result_data.drop(
                    ['FILTER', 'BLACKLIST', 'GeneDetail.refGene'], axis=1, inplace=True)

                self.log.info('writting result to file %s', out)
                result_data.to_csv(
                    out, index=False, encoding='utf-8', sep='\t')
            else:
    ...
    def usage(self):
        ...
        '''
        print(
            'Usage : ./GermlineSnvAnnotationFilter [OPTION]... 或者 python GermlineSnvAnnotationFilter [OPTION]')
        print('''
            根据vcf文件，annovar注释输出结果，输出最终分析结果文件，格式为.xls
            Example:\t
                ./GermlineSnvAnnotationFilter.py \\
                    --database=/opt/deaf/blacklist.csv \\
                    -v result/B1701_filtered_snpeff.vcf \\
                    -a result/B1701.hg19_multianno.txt \\
                    -o result/B1701.result.xls
            或者\t
                python GermlineSnvAnnotationFilter.py \\
                    --database=/opt/deaf/blacklist.csv \\
